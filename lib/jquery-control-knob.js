...
(function (window, $, undef) {
    ...
    $.createControlKnob = function (options) {
        ...
        function val(user_val) {
            max_sub = getComputedMax();
            if (arguments.length > 0) {
                user_val = Number(user_val) || 0;
                if (user_val > max_sub) {
                    user_val = max_sub;
                }
                if (user_val < min_value) {
                    user_val = min_value;
                }
                value = user_val;
        ...
        (function () {
            ...
            */
            ...
            function ckAnchorEventHandler(event) {
                event.preventDefault();
                switch (event.type) {
                case 'touchstart':
                    event.pageX = event.originalEvent.touches[0].pageX;
            ...
            function ckWrapEventHandler(event) {
                ...
                case 'mousedown':
                    event.preventDefault();
                    $control_knob_object.triggerHandler('mousedown');
                    break;
                case 'keydown':
            ...
            control_knob_object.enable = function enable() {
                if (disabled === true) {
                    ...
                    $ck_wrap
                        ...
                        .on('mousedown keydown', ckWrapEventHandler)
                        .attr('tabindex', tab_index);
                    $ck_anchor
                        .off('mousedown', enableDisableAid)
                        .on('mousedown touchstart mousewheel mousewheel keydown', ckAnchorEventHandler);
                }
                return control_knob_object;
            };
            control_knob_object.disable = function disable() {
                if (disabled === false) {
                    ...
                    $ck_wrap
                        .addClass('disabled')
                        .off('mousedown keydown', ckWrapEventHandler)
                        .removeAttr('tabindex');
                    $ck_anchor
                        .off('mousedown touchstart mousewheel mousewheel keydown', ckAnchorEventHandler)
